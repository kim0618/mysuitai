...
function schema(entry,payload={}){
  ...
  if(op==='moveColumn'&&(!Number.isInteger(Number(payload.toVisualIndex))||Number(payload.toVisualIndex)<0||Number(payload.toVisualIndex)>(payload.staticColumnCount?payload.staticColumnCount-1:6)))return invalid('INVALID_PAYLOAD','toVisualIndex는 0~6 정수여야 합니다.',{field:'toVisualIndex'});
  if(['moveRenderedRow','moveRow'].includes(op)&&(!Number.isInteger(Number(payload.toRowIndex))||Number(payload.toRowIndex)<0||Number(payload.toRowIndex)>99))return invalid('INVALID_PAYLOAD','toRowIndex는 0~99 정수여야 합니다.',{field:'toRowIndex'});
...
function fromStructure(body){let targetType='LOGICAL_COLUMN',target={logicalColumnKey:body.logicalColumnKey,logicalTableKey:body.logicalTableKey,columnIndex:body.columnIndex},payload={};if(['resizeRow','hideRow','collapseRow','restoreRow','moveRenderedRow','moveRow','removeRow'].includes(body.operation)){targetType='LOGICAL_ROW';target={logicalRowKey:body.logicalRowKey}}else if(['moveTable','resizeTableWidth','hideTable','collapseTable','restoreTable'].includes(body.operation)){targetType='COMPOSITE_TABLE';target={compositeTableKey:body.compositeTableKey}}if(body.operation==='resizeColumn')payload.width=body.afterWidth;if(body.operation==='moveColumn')payload={toVisualIndex:body.toVisualIndex,groupKey:body.groupKey,...(String(body.logicalTableKey||'').startsWith('tbl:static:')?{staticColumnCount:100}:{})};if(body.operation==='resizeRow')payload.height=body.afterHeight;if(['moveRenderedRow','moveRow'].includes(body.operation))payload.toRowIndex=body.toRowIndex;if(body.operation==='moveTable')payload.after=body.after;if(body.operation==='resizeTableWidth')payload.width=body.afterWidth;return{...body,draftId:body.layoutDraftId,targetType,target,payload}}
